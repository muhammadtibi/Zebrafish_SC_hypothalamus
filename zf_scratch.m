...
load('/data/Technion_analysis/zebrafish/listy.mat')
...
%         data = fillmissing(data, 'constant', 0);
    ...
    all_flags(1).fc_time=zeros(size(data,2),1);
...
%         in= 1:size(data,2);
        ...
        elseif fetsel=="InterSpecies"
            ...
            in_b = find(sum(data(:,batchid==0)>0,2)>5 & sum(data(:,batchid==0)>0,2)<length(data(1,batchid==0))*0.5 & ~ismember(geneid(:,1),ex_genes));
...
% %     end
    ...
    save('Clust_param','clmethod','nrmz','MinPts','eps_prc','ex_genes','ex_clust','list2','maximal_total_molecule','initial_dims','perplexity','valm','valg','valmx','perct_90th_molecule','listy','datex','colors','fct','findx','sample_uni','all_legflag','filtery','fetsel','batcorr','-v7.3');
    %
    disp('Saving Orignal...')
