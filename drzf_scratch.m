...
try
    ...
    S = {...
        ...
        struct('name','Fet. Sel.','type','enum','values',{{'InterSpecies','Default','Alt','Spatial'}});...
...
direct='/data/Technion_analysis/zebrafish/drzf_040922';%uigetdir('/data/Technion_analysis','Please choose MAIN analysis folder');% get save folder
...
%         end
        ...
        clear all_flags;
        all_flags(1).fc_time=all_flags_sorted(validcells)
...
% %             listyx=uipickfiles('num',[],'FilterSpec',[],'out','struct'); % loop images
        ...
        cd('/data/runs/samples')
%         listyx=uipickfiles('num',[],'FilterSpec',[],'out','struct'); % loop images
           load('/data/Technion_analysis/zebrafish/listy.mat', 'listyx')
...
%     geneid=regexprep(geneid,'SLC17A6B','SLC17A6');
        ...
        % addpath(genpath(direct));
                %% add maxwells dr dataset 
                disp("load drzf")
          load('/data/Technion_analysis/zebrafish/zfdr.mat')
          load('/bigdata/zebrafish_maxwell/dr_maxwell_neurons.mat','dr_genes','dr_matrix','dr_cellid','dr_cellname','dr_sample')
        %%  concatante zf with dr
       all_flags(1).fc_time=[ones(length(sample),1);zeros(length(dr_sample),1)]; 

        [inBoth,xi,xf] = intersect(geneid, upper(dr_genes)) ;

        dr_matrix=dr_matrix(xf,:);
        data=data(xi,:);
        % 
        geneid=inBoth;
        data=[data,dr_matrix];
        cellid=[cellid;dr_cellid];
        sample=[sample;dr_sample];
        geneid_org=geneid;
        data_org=data;
        cellid_org=cellid;
        sample_org=sample;
...
%         in= 1:size(data,2);
        ...
        elseif fetsel=="InterSpecies"
            ...
            in_b = find(sum(data(:,batchid==0)>0,2)>5 & sum(data(:,batchid==2)>0,2)<length(data(1,batchid==0))*0.5 & ~ismember(geneid(:,1),ex_genes));
            corr_filt1 = cv_vs_m_selection(data(in_m,batchid==1),geneid(in_m),[],1,0);
            corr_filt2 = cv_vs_m_selection(data(in_b,batchid==0),geneid(in_b),[],1,0);
            % %             corr_filt = cv_vs_m_selection(data(in,:),geneid(in),[],1,0);  % org
...
%     initial_dims = 30;
 ...
 prj=prjx;
  ...
  if savefig_flag==1
    ...
    prj=prj_all{4};% {itr+1} % CHANGE HERE- smae as in gr!!!!!!!!!!!!!!!!!!!!!!!
    cd(direct)
    disp('projecting...')
...
%                     set(ht,'BackgroundColor',0.8*[1,1,1],'fontsize',5);
                elseif i==0
                    ii=find(idx==i);
                    h=plot(mapped_xy(ii,1),mapped_xy(ii,2),'+','color',[0.5 0.5 0.5],'markersize',2); hold on;
...
%                     filtery=input('Filter:');
    ...
    samples_idx=string(unique(sample));
    for ii=starter:size(all_flags,2)
        fc_uni=1 ;%unique(all_flags(ii).fc_time_sorted)
        fl_name=string(fc_uni);
...
%             legend(leg(unique(gabaglut_sc)))
    ...
    save('Clust_param','clmethod','nrmz','MinPts','eps_prc','ex_genes','ex_clust','list2','maximal_total_molecule','initial_dims','perplexity','valm','valg','valmx','perct_90th_molecule','listy','datex','colors','fct','findx','sample_uni','filtery','fetsel','batcorr','-v7.3');
    %
    disp('Saving Orignal...')
